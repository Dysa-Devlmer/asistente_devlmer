/**
 * Empleado Service - Bootstrap API v2.0
 *
 ...
 * - GARANTIZAR que NO se exponen campos sensibles
 */

import { EmpleadoRepository } from '../repositories/empleado.repository';
import { EmpleadoResponseDataDto } from '../dto/empleado.dto';
...
export class EmpleadoService {
  ...
  constructor(db: any) {
    this.empleadoRepository = new EmpleadoRepository(db);
  }
   ...
   * @returns Response wrapper con empleados paginados
  ...
  ): Promise<BootstrapResponseWrapper<EmpleadoResponseDataDto>> {
    // 1. Validar paginación
    this.validatePagination(page, limit);

    // 2. Verificar que sucursal existe
    const sucursalExists = await this.empleadoRepository.sucursalExists(sucursalId);
    if (!sucursalExists) {
      throw new Error(`Sucursal con ID ${sucursalId} no encontrada o inactiva`);
    }

    // 3. Obtener empleados desde repository
    const empleadoData = await this.empleadoRepository.getEmpleados(sucursalId, page, limit);

    // 4. Construir response data
    const responseData: EmpleadoResponseDataDto = {
      sucursal_id: sucursalId,
      pagination: empleadoData.pagination,
      empleados: empleadoData.empleados,
    };
