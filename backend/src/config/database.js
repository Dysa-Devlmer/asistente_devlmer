...
const initializeTables = async () => {
  try {
    ...
    await db.schema.createTable('cash_movements', (table) => {
      table.increments('id').primary();
      table.integer('cash_session_id').unsigned().notNullable().references('id').inTable('cash_sessions').onDelete('CASCADE');
    ...
    await db.schema.createTable('z_reports', (table) => {
      ...
      table.integer('cash_session_id').unsigned().notNullable().references('id').inTable('cash_sessions').onDelete('RESTRICT');
